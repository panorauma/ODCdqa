        ...
        mainPanel(
          tabsetPanel(
            ...
            tabPanel("Dataset",
              value = "dataset",
              div(
                style = "overflow-x:scroll;height:95%;",
            ...
            tabPanel("AutoFilled Data Dictionary",
              value = "autodatadic",
              div(
                style = "overflow-x:scroll;height:95%;",
...
server <- function(input, output, session) {
  ...
  observeEvent(input$checkButton, {
    ...
    if (input$community == "SCI") {
      dataframes$valid <- validate_odc(
        dataset = dataframes$df_data,
  ...
  output$result_table <- DT::renderDT({ # convert Data check tab to DT
    ...
    dataframes$render_checks <- render_check_table(
      dataframes$valid$structure,
  ...
  output$suggested_varname <- DT::renderDT({
    ...
    names(suggested_varname) <- c(
      "Variable name", "Variable name (# characters)",
    ...
    showModal(
      modalDialog(
        ...
        HTML(
          paste(
            "<p>",
            "<strong>Check name: </strong>", temp_row[["check_name"]]
          ), "<br><br>",
