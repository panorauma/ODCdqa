...
checklist_to_df <- function(check_list) { # MARK: checklist_to_df
  ...
  table_report$Name_blank <- str_replace_all(
    table_report$Name_blank,
    c("," = "", "c[(]" = "", "[)]" = "")
  )
...
empty_data_dict <- function(upload_dataset) { # MARK: empty_data_dict
  ...
  # create empty df
  autoDataDic <- data.frame(
    VariableName = unlist(df_data_Names),
    Title = NA,
    Unit_of_Measure = NA,
    Description = NA,
...
render_check_table <- function(structure_checks, schema_checks) { # MARK: render_check_table
  ...
  temp_structure$CheckName <- c(
    "Blank header", "Duplicated header", "Blank row",
  ...
  table_html <- DT::datatable(table_report,
    ...
    rownames = FALSE,
    colnames = c(
      "Type of check", "Check name", "Status",
...
summary.odcCheck <- function(validate_results) { # MARK: summary.odcCheck
  ...
  if ("blank_header" %in% names(a$structure)) {
    if (a$structure$blank_header$n_blank_header == 0) {
      ...
      cat(
        "*Blank Header:", a$structure$blank_header$n_blank_header,
        "error/s detected", "\n"
      )
      cat(
        "\t", "Blank Header number:",
  ...
  if ("duplicated_header" %in% names(a$structure)) {
    if (a$structure$duplicated_header$n_dupli_header == 0) {
      ...
      cat(
        "*Duplicated Header:", a$structure$duplicated_header$n_dupli_header,
        "error/s detected", "\n"
      )
      cat(
        "\t", "Duplicated Headers:",
  ...
  if ("blank_row" %in% names(a$structure)) {
    if (a$structure$blank_row$n_blank_row == 0) {
      ...
      cat(
        "*Blank row:", a$structure$blank_row$n_blank_row,
        "error/s detected", "\n"
      )
      cat(
        "\t", "Rows with all blank fields:",
  ...
  if ("blank_column" %in% names(a$structure)) {
    if (a$structure$blank_column$n_blank_column == 0) {
      ...
      cat(
        "*Blank column:", a$structure$blank_column$n_blank_column,
        "error/s detected", "\n"
      )
      cat(
        "\t", "Columns with all blank fields:",
  ...
  if ("duplicated_row" %in% names(a$structure)) {
    if (a$structure$duplicated_row$n_dupli_row == 0) {
      ...
      cat(
        "*Duplicated row:", a$structure$duplicated_row$n_dupli_row,
        "error/s detected", "\n"
      )
      cat(
        "\t", "Rows duplicated:",
  ...
  if ("duplicated_column" %in% names(a$structure)) {
    if (a$structure$duplicated_column$n_dupli_column == 0) {
      ...
      cat(
        "*Duplicated column:", a$structure$duplicated_column$n_dupli_column,
        "warning/s detected", "\n"
      )
      cat(
        "\t", "Columns duplicated:",
  ...
  if ("minimal_var" %in% names(a$structure)) {
    if (a$structure$minimal_var$n_missing_minimal_var == 0) {
      ...
      cat(
        "*Requiered variables:", a$structure$minimal_var$n_missing_minimal_var,
        "error/s detected", "\n"
      )
      cat(
        "\t", "Missing requiered variables:",
  ...
  if ("data_dic_headers" %in% names(a$schema)) {
    if (a$schema$data_dic_headers$n_datadic_Noheaders == 0) {
      if (!is.null(a$schema$data_dic_headers$data_dic_header_in_order) &&
        ...
        cat(
          "\t", "Data dictionary headers are in the wrong order, make sure the order is:",
          paste(c(
            "VariableName", "Title", "Unit_of_Measure", "Description",
      ...
      cat(
        "*Data dictionary header:", a$schema$data_dic_headers$n_datadic_Noheaders,
        "error/s detected", "\n"
      )
      cat(
        "\t", "Data dictionary header missing:",
  ...
  if ("data_dic_headers" %in% names(a$schema)) {
    if (!is.list(a$schema$extra_header)) {
      ...
      cat(
        "*Data extra header:", a$schema$extra_header$n_extra_header,
        "error/s detected", "\n"
      )
      cat(
        "\t", "Data extra header:",
  ...
  if ("missing_header" %in% names(a$schema)) {
    if (!is.list(a$schema$missing_header)) {
      ...
      cat(
        "*Data missing header:", a$schema$missing_header$n_missing_header,
        "error/s detected", "\n"
      )
      cat(
        "\t", "Data missing header:",
  ...
  if ("missing_description" %in% names(a$schema)) {
    if (!is.list(a$schema$missing_description)) {
      ...
      cat(
        "*Missing description:", a$schema$missing_description$n_missing_des,
        "error/s detected", "\n"
      )
      cat(
  ...
  if ("missing_title" %in% names(a$schema)) {
    if (!is.list(a$schema$missing_title)) {
      ...
      cat(
        "*Missing title:", a$schema$missing_title$n_missing_title,
        "error/s detected", "\n"
      )
      cat(
  ...
  if ("missing_minimal_var" %in% names(a$schema)) {
    if (a$schema$missing_minimal_var$n_missing_minimal_var == 0) {
      ...
      cat(
        "*Required variables:", a$schema$missing_minimal_var$n_missing_minimal_var,
        "error/s detected", "\n"
      )
      cat(
...
validate_odc <- function(dataset = NULL, datadic = NULL,
  ...
  if (is.null(datadic_path) & is.null(datadic)) {
    ...
    if (!is.null(dataset_path) & !is.null(datadic_path)) {
      ...
      results[["schema"]] <- validate_schema(
        dataset = dataset, datadic = datadic,
        sch_checks = sch_checks
      )
    }
  }
...
validate_schema <- function(dataset, datadic, sch_checks = "all") {
  ...
  if ("miss_description" %in% checks) {
    if (!"Description" %in% datadic_headers) {
      ...
      results[["missing_description"]] <- list(
        "n_missing_des" = sum(datadic_des == "" | is.na(datadic_des)),
        "which_missing_des" =
  ...
  if ("miss_title" %in% checks) {
    if (!"Title" %in% datadic_headers) {
      ...
      results[["missing_title"]] <- list(
        "n_missing_title" = sum(datadic_title == "" | is.na(datadic_title)),
        "which_missing_title" =
  ...
  if ("min_vars" %in% checks) {
    results[["missing_minimal_var"]] <- list(
      "n_missing_minimal_var" =
  ...
  # Check if variable name is over 64 characters
  if ("over_64char" %in% checks) {
    # check each datadic_vars & store in df
    char_length_check <- lapply(datadic_vars, function(x) {
      ifelse(nchar(x) > 64, "Fail", "Pass")
    })
    ...
    # store results in nested lists
    results[["over_64char"]][["n_over_64char_headers"]] <- nrow(char_length_df)
    results[["over_64char"]][["which_over_64char_headers"]] <- unlist(char_length_df$Key)
...
validate_structure <- function(dataset, str_checks = "all") { # MARK: validate_structure
  ...
  if ("blank_header" %in% checks) {
    ...
    results[["blank_header"]] <- list(
      "n_blank_header" = sum(name_header == ""),
  ...
  if ("dup_header" %in% checks) {
    ...
    results[["duplicated_header"]] <- list(
      "n_dupli_header" = sum(duplicated_header > 1),
